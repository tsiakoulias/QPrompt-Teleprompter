# [702-QTD-02] QQuickTextDocument destroyed without destroyed signal connection

- **Status:** NEEDS-INFO
- **Severity:** Low-Medium
- **Category:** 
- **Location:** `documenthandler.h:334, documenthandler.cpp:175-202`
- **Consensus:** 3/6 agents LEGIT · split

## Original report claim

- **File:** documenthandler.h:334, documenthandler.cpp:175-202
- **Severity:** Low-Medium
- **Analysis:** m_document is raw QQuickTextDocument* from QML. No destroyed signal to null it. If TextEdit destroyed before DocumentHandler, m_document dangles. Composes with QTD-01.
- **Impact:** Dangling m_document during destructor if QML cleanup order unfavorable.

### [META-01–12] Desktop/AppData metadata bugs (12 total)
- Invalid category `Qt` in desktop file (not a registered freedesktop.org category)
- MimeType mismatch: desktop has `text/html` only, AppData has `text/plain`, `text/markdown`, `text/html`
- Spam keyword `imaginary` in desktop file
- Typo `fixedd` (double-d) in v2.0.2 release notes
- 11 releases use non-ISO-8601 dates (YYYY-M-D instead of YYYY-MM-DD)
- XML indentation inconsistency at v1.1.5 release tag
- Missing checksum on v2.0.2 source artifact
- v1.0 missing checksums on 4/5 artifacts
- Version `1.1` vs GitHub tag `v1.1.0` mismatch
- Version `1.0` vs GitHub tag `v1.0.0` mismatch
- Artifact URLs use `QPrompt` repo name, everything else uses `QPrompt-Teleprompter` → 404

## Agent assessments

| Agent | Verdict | Conf | Rationale |
|---|---|---|---|
| opus-4.8-extra | ❔ UNSURE | 40 | QQuickTextDocument destroyed-signal not connected; lifetime QML-managed (documenthandler.cpp:175) |
| gpt-5.5-xhigh | ✅ LEGIT | 78 | QQuickTextDocument destroyed without destroyed signal connection (src/documenthandler.h:334) |
| deepseek-v4-pro-max | ✅ LEGIT | 85 | Raw QQuickTextDocument* m_document at documenthandler.h:334 — no destroyed signal dangles on QML teardown |
| glm-5.2-xhigh | ⚠️ PARTIAL | 60 | documenthandler.h:334 QQuickTextDocument destroyed without destroyed signal connection; potential dangling |
| kimi-k2.7-code | ✅ LEGIT | 80 | documenthandler.h:334 stores QQuickTextDocument* raw; no destroyed handler to null it on QML teardown. |
| opus-4.8-ultra | ❔ UNSURE | 40 | max: unverifiable statically (runtime/semantic) — QQuickTextDocument destroyed-signal not connected; lifetime QML-managed (documenthandler.c |

## Patch  _(fill when fixing)_

- **Root cause:**
- **Fix:**
- **Files changed:**
- **Verification:**
- **Commit / PR:**

